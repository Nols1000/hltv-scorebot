	...
	socket.on('connect', function (res) {
		...
		if (!reconnected) {
            ...
            socket.on('score', function (s) {
				...
				ee.emit("score-update", s.tScore, s.ctScore);
            });
...
module.exports.getPlayerByName = function(name) {
	...
	for(var k = 0; k < player.t.length; k++) {
		
		if(name.indexOf(player.h[k].name) != -1) {
...
function updateAllPlayer() {
	...
	for(var i = 0; i < player.h.length; i++){
		
		updatePlayerList(player.h[i]);
	}
...
function resetPlayerDeathAttr() {
	...
	for(var k = 0; k < player.h.length; k++) {
	
		player.h[k].death = false;
	}
}
...
function updatePlayerList(p) {
	...
	if(module.exports.getPlayerByName(p.name) != null) {
		...
		for(var k = 0; k < player.t.length; k++) {
			...
			if(p.name.indexOf(player.h[k].name) != -1) {
